...
void main() {
  ...
  // Longer wait used after bookmark API calls and filter switches
  Future<void> waitForBackendSlow(WidgetTester tester) async {
    await tester.pump();
  ...
  Future<int> bookmarkOneWorkout(WidgetTester tester) async {
    ...
    while (bookmarkTapCount < 1) {
      final unsavedBookmarkIcons = find.byIcon(Icons.bookmark_border_rounded);

      if (unsavedBookmarkIcons.evaluate().isNotEmpty) {
        await tester.tap(unsavedBookmarkIcons.first);
        await tester.pump();
        // Increased from 1s to 3s to allow the bookmark API call to complete
        await tester.pump(const Duration(seconds: 3));
        await tester.pumpAndSettle(const Duration(milliseconds: 500));
        bookmarkTapCount++;
        print(
          '  |- [PASS] User pressed bookmark icon #$bookmarkTapCount',
        );
        continue;
      }
  ...
  group('Workout Catalogue - Bookmark Workouts [TC-WO-004]', () {
    testWidgets('user can bookmark one workout and see it with Saved filter', (
      ...
      verifyOneWidget(1, 'Workout page header "Train" is visible', find.text('Train'));
      ...
      logStep(4, 'User melihat semua workout yang di-bookmark dengan filter Saved.');
      await scrollWorkoutListToTop(tester);

      // Use a more specific finder scoped to avoid accidentally tapping
      // a different 'Saved' text elsewhere in the widget tree (e.g. bottom nav)
      final savedFilter = find.text('Saved').last;
      await tester.ensureVisible(savedFilter);
      await tester.tap(savedFilter);
      // Use the slower wait here — the filter refetch needs more time,
      // especially right after a bookmark write on a fresh account
      await waitForBackendSlow(tester);
